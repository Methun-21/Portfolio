...
export default function App() {
  return (
    <div className="min-h-screen bg-[#030303] text-white selection:bg-blue-500/30 relative overflow-hidden font-sans">
      {/* Mesh Background */}
      <div className="fixed inset-0 z-0 mesh-gradient animate-slow-pan opacity-60 pointer-events-none" />
      
      {/* Navigation */}
      <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-50 px-8 py-4 glass-nav rounded-full flex items-center gap-12 shadow-2xl shadow-black/50 border border-white/5 backdrop-blur-xl">
        <span className="font-black tracking-tighter text-xl bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-500">METHUNRAJ.A</span>
        ...
        <div className="flex gap-4">
          <a href="https://github.com/Methun-21" target="_blank" className="hover:scale-110 hover:text-blue-400 transition-all"><Github size={18} /></a>
          <a href="https://www.linkedin.com/in/methunraj-a-6a5aa932a" target="_blank" className="hover:scale-110 hover:text-blue-400 transition-all"><Linkedin size={18} /></a>
        </div>
      </nav>

      <main className="relative z-10 pt-48 px-6 max-w-7xl mx-auto pb-32">
        {/* Hero Section */}
        <section className="text-center mb-48 flex flex-col items-center">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1, ease: "easeOut" }}
          >
            <div className="inline-flex glass-pill mb-8 text-blue-400 shadow-[0_0_30px_rgba(59,130,246,0.3)] border border-blue-500/20 px-6 py-2 rounded-full backdrop-blur-md">
              <Sparkles size={14} className="mr-2" />
              <span className="text-sm font-semibold tracking-wide">Available for New Challenges</span>
            </div>
            <h1 className="text-6xl md:text-[9rem] font-black tracking-tighter leading-[0.85] mb-12 text-glow">
              CRAFTING <br />
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-white via-white/80 to-white/20">INTELLIGENCE.</span>
            </h1>
            ...
            <div className="flex flex-wrap justify-center gap-6">
              <a href="#work" className="px-10 py-5 bg-white text-black font-black rounded-full hover:scale-105 hover:shadow-[0_0_40px_rgba(255,255,255,0.3)] transition-all duration-300 flex items-center gap-3">
                Explore Work <ArrowUpRight size={20} />
              </a>
              <a href="mailto:methunraja7393@gmail.com" className="px-10 py-5 glass-card font-black rounded-full hover:bg-white/10 hover:border-white/20 transition-all duration-300 flex items-center gap-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {PROJECTS.map((project, i) => (
              <motion.div 
                ...
                className="glass-card p-10 md:p-12 group cursor-pointer relative overflow-hidden rounded-[2.5rem] border border-white/5 hover:border-white/20 transition-colors"
              >
                ...
                <div className="relative z-10 h-full flex flex-col">
                  ...
                  <p className="text-white/50 text-lg leading-relaxed mb-10 font-medium flex-grow">
                    {project.desc}
                  </p>
          ...
          >
            <div className="glass-card p-10 rounded-[2.5rem] border border-white/5 flex-1">
              ...
              <div className="flex flex-wrap gap-2">
                {TECH_STACK.map(s => (
                  <span key={s} className="px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full text-[11px] font-bold text-white/70 transition-colors cursor-default">{s}</span>
                ))}
              </div>
            </div>
            ...
            <div className="glass-card p-10 rounded-[2.5rem] border border-white/5 flex-1">
              ...
              <ul className="space-y-4">
                {CERTIFICATIONS.map((cert, i) => (
                  <li key={i} className="text-sm text-white/70 font-medium flex items-start gap-3">
          ...
          >
            ...
            <div className="flex flex-col items-center gap-10">
              <a href="mailto:methunraja7393@gmail.com" className="text-2xl md:text-4xl font-black text-white hover:text-blue-400 transition-colors relative group">
                ...
                <span className="absolute -bottom-2 left-0 w-0 h-1 bg-blue-500 group-hover:w-full transition-all duration-300" />
              </a>
              
              <div className="flex gap-6">
                <a href="https://github.com/Methun-21" target="_blank" className="p-4 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/20 hover:scale-110 hover:-translate-y-1 transition-all duration-300 text-white/70 hover:text-white">
                  <Github size={24} />
                </a>
                <a href="https://www.linkedin.com/in/methunraj-a-6a5aa932a" target="_blank" className="p-4 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/20 hover:scale-110 hover:-translate-y-1 transition-all duration-300 text-white/70 hover:text-[#0A66C2]">
                  <Linkedin size={24} />
                </a>
              </div>
            </div>
